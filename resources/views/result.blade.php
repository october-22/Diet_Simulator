...
<body style="background-color: #E5FFCC;">
    <div class="container mt-5">
        ...
        <div class="form-container bg-white p-4 rounded shadow">
            ...
            <div class="result">{{ $bmr }} <span class="unit">kcal</span></div>

            <h2>活動代謝</h2>
            <p>運動や普段の生活で消費されたカロリー</p>
            <div class="result">{{ $activeMetabolism }} <span class="unit">kcal</span></div>

            <h2>一日の消費カロリー</h2>
            <p>基礎代謝 + 活動代謝 = 一日の総カロリー消費</p>
            <div class="result">{{ $dailyMetabolism }} <span class="unit">kcal</span></div>

            <h2>一日の減量値</h2>
            <p>一日の消費 - 一日の摂取 = 減量値</p>
            <div class="result" @if ($dailyCalorieDeficit < 0) style="color: #CC0000;" @endif>
                @if (-$dailyCalorieDeficit > 0)
                    +{{ -$dailyCalorieDeficit }} <span class="unit">kcal</span>
                @else
                    {{ -$dailyCalorieDeficit }} <span class="unit">kcal</span>
                @endif
            </div>
            ...
            <div class="result"
                @if ($weightAfterDuration > $nowWeight) style="color: #CC0000;"
                @elseif ($weightAfterDuration == $nowWeight) style="color: #FF8000;" 
                @endif>
                {{ $nowWeight }} ⇒ {{ $weightAfterDuration }} <span class="unit">kg</span>
            </div>
